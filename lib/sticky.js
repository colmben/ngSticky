...
(function() {
   ...
   * Directive: sticky
   */
  mod.directive('sticky', ['$window', '$timeout', function($window, $timeout) {
      return {
        restrict: 'A', // this directive can only be used as an attribute.
        link: function linkFn($scope, $elem, $attrs) {
            // Sub the $elem first if required - remember we also had to take out the isolated scope which allowed
            // the disabled-sticky attr to work
            if ($attrs.stickyTargetElement){
                var target = $elem.find($attrs.stickyTargetElement)[0];
                $elem = angular.element(target) || $elem;
            }
